        ...
        DispatchQueue.main.async {
            if let error = Error {
                ...
                do {
                    ...
                    let tokenDetails = jsDataResult["t"] as! [String : Any];
                    completion?(jsDataResult["ok"] as! Bool, Token(userId: tokenDetails["userId"] as? String, token: token, expires: Date.init()), nil);
